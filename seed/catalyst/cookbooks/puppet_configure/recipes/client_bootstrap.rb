...
pup_master_fqdn = pup_config['puppet_master']['pem_file']

puppet_client_cmd = "puppetd --test --server #{pup_master_fqdn} --waitforcert 10  > /dev/null 2>&1 &"


if pup_config['client']['ssh_pass_method']
	client_ssh_cmd = "sshpass -p '#{pup_client_ssh_pswd}' ssh -o 'StrictHostKeyChecking no' -t #{pup_client_ssh_user}@#{pup_client_ip}"
	client_scp_cmd = "sshpass -p '#{pup_client_ssh_pswd}' scp"
else
	client_ssh_cmd = "ssh -i #{pup_client_pem_file}-o 'StrictHostKeyChecking no' -t #{pup_client_ssh_user}@#{pup_client_ip}"
	client_scp_cmd = "scp -i #{pup_client_pem_file}"
end

if pup_config['puppet_master']['ssh_pass_method']
	master_ssh_cmd = "sshpass -p '#{pup_master_ssh_pswd}' ssh -o 'StrictHostKeyChecking no' -t #{pup_master_ssh_user}@#{pup_master_ip}"
	master_scp_cmd = "sshpass -p '#{pup_master_ssh_pswd}' scp"
else
	master_ssh_cmd = "ssh -i #{pup_master_pem_file}-o 'StrictHostKeyChecking no' -t #{pup_master_ssh_user}@#{pup_master_ip}"
	master_scp_cmd = "scp -i #{pup_master_pem_file}"
end
...
template '/tmp/solo.rb' do
	...
	source 'solo.rb.erb'
	mode 00755
	variables(:path => pup_config['cache_dir'])
end
...
execute "Bootstrap Node" do
	command "#{client_ssh_cmd} 'sudo chef-solo -o recipe[puppet_configure::client_config]' "
	action :run
end
...
execute "Executing puppet_cert_accept command on Puppet Master #{pup_master_ip} for #{pup_client_fqdn}" do
	command "#{master_ssh_cmd} 'sudo /usr/sbin/puppetca --sign #{pup_client_fqdn}' "
	action :run
end